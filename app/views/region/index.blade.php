...
<div id="content-header">
    <div id="breadcrumb"><a href="index.html" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> 控制面板</a>
        <a href="#" class="tip-bottom">系统管理</a> <a href="#" class="current">{{$config['title']}}</a></div>
    <!--    <h1>级别列表</h1>-->
</div>
<br>
...
<div class="row-fluid">
	<div class="span12">
		...
		<p style="float: right;">
            ...
            @if(isset($config['next_router']))
            <button id="look" class="btn btn-success"><i class="icon-eye-open"></i></i>  查看下级内容</button>
            @endif
        </p>
	</div>
</div>
    ...
    $(function () {
        ...
        $(":button[id='look']").click(function () {
            ...
            if ($.trim(checks) == ''||num>1) {
                ...
                @if(isset($config['next_router']))
                location.href = "{{ URL::to($config['next_router']) }}?father_id=" + checks;
                @endif
            }
        });
